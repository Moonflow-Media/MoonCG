...
MoonCG is a broadcast graphics framework. This codebase includes:
...
- **Workspaces**: `workspaces/*` (mooncg, internal-util, database-adapters, cli)
  - Main package is in `workspaces/mooncg/`
- **Source**: `workspaces/mooncg/src/{server,client,shared}`
- **Tests**: `workspaces/mooncg/test/**/*.test.ts` (E2E), `workspaces/mooncg/src/**/*.test.ts` (unit)
- **Build output**: `workspaces/mooncg/dist/` (bundled server/client code)
- **Root**: Minimal wrapper with `mooncgRoot: true` field, `index.js` wrapper pointing to workspace

## Build System

- `npm run build` uses `tsdown` bundler to compile and bundle all workspaces
- **Output**: `workspaces/mooncg/dist/` (bundled server and client code as CommonJS)
...
- **Test setup**: `workspaces/mooncg/test/helpers/setup.ts` creates isolated temp directories per test file
- **E2E fixtures**: Browser pages are lazy-loaded and reused within test file
- **Working directory**: Vitest runs from repo root, so all test paths must be `workspaces/mooncg/test/...`

### Test Fixtures

- **Location**: `workspaces/mooncg/test/fixtures/mooncg-core/`
...
- `workspaces/mooncg/test/helpers/setup.ts`: Creates MoonCG server + Puppeteer browser per test file
...
- Fixture paths in tests must be relative to repo root: `workspaces/mooncg/test/fixtures/...`
...
- **Dashboard routes**: `/node_modules/` route serves from mooncg installation to bundles
...
- Workspace packages: `@mooncg/internal-util`, `@mooncg/database-adapter-sqlite-legacy`
- TypeScript: CommonJS (`module: "commonjs"`), ES2022 target

### Module Resolution

- **Always use `.js` extensions** in imports, even for `.ts` source files
- Workspace packages (`@mooncg/*`) resolve via npm workspaces
...
- Dynamic port allocation: `process.env.MOONCG_TEST` → OS assigns random port
...
- **Legacy mode**: Root package has `mooncgRoot: true`, bundles in `bundles/` directory
- **Installed mode**: MoonCG installed as dependency in `node_modules`, project root IS the bundle
- Mode determined by `isLegacyProject` check (`package.json` has `mooncgRoot: true`)
- **Critical**: `@mooncg/internal-util` caches `rootPath` and `isLegacyProject` at module load time
- Tests must set `process.env.MOONCG_ROOT` BEFORE importing any MoonCG modules
- Use `getMooncgRoot()` function (respects MOONCG_ROOT) instead of `rootPath` constant where appropriate

## Effect-TS Migration

MoonCG is incrementally migrating to Effect-TS. See `docs/effect-migration/` for strategy and plans.
...
- Importing MoonCG modules before setting `MOONCG_ROOT` env var (causes wrong root path to be cached)
