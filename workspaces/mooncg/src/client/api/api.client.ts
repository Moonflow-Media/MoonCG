...
>();
const apiContexts = new Set<MoonCGAPIClient<Record<string, any>>>();
...
export class MoonCGAPIClient<
	C extends Record<string, any> = MoonCG.Bundle.UnknownConfig,
> extends MoonCGAPIBase<"client", C, EventMap> {
	static Replicant<
		V,
	...
	get Logger(): new (name: string) => MoonCG.Logger {
		return Logger;
	}

	get log(): MoonCG.Logger {
	...
	/**
	 * A filtered copy of the MoonCG server config with some sensitive keys removed.
	 */
	...
	private readonly _soundFiles?: ClientReplicant<MoonCG.AssetFile[]>;

	private readonly _bundleVolume?: ClientReplicant<number>;

	private readonly _masterVolume?: ClientReplicant<number>;

	private _soundCues: MoonCG.SoundCue[] = [];

	private _memoizedLogger?: MoonCG.Logger;

	constructor(
		bundle: MoonCG.Bundle & { _hasSounds?: boolean },
		...
		if (globalThis.window && bundle._hasSounds && window.createjs?.Sound) {
			const soundCuesRep = new ClientReplicant<MoonCG.SoundCue[]>(
				"soundCues",
				this.bundleName,
				{},
				socket,
			);
			this._soundFiles = new ClientReplicant<MoonCG.AssetFile[]>(
				"assets:sounds",
				this.bundleName,
				{},
				socket,
	 ...
	 */
	findCue(cueName: string): MoonCG.SoundCue | undefined {
	 ...
	 * @param [opts] {Object}
	 * @param [opts.updateVolume=true] - Whether or not to let MoonCG automatically update this instance's volume
	...
	playSound(
		...
		if (!globalThis.window) {
			throw new Error("MoonCG Sound API methods are not available in workers");
		}
		...
		if (!window.createjs?.Sound) {
			throw new Error(
				"MoonCG Sound API methods are not available when SoundJS isn't present",
			);
		}
	...
	sendMessageToBundle<T = unknown>(
		...
		return MoonCGAPIClient.sendMessageToBundle(
			messageName,
			bundleName,
			dataOrCb,
			cb as any,
	...
	readReplicant<T = unknown>(
		...
		MoonCGAPIClient.readReplicant<T>(name, namespace, cb);
	}
	...
	private _updateInstanceVolumes(): void {
		...
		this._soundCues.forEach((cue) => {
			instancesArr.forEach((instance) => {
				const meta = soundMetadata.get(instance);
				if (meta?.cueName === cue.name && meta.updateVolume) {
					this._setInstanceVolume(instance, cue);
				}
			});
		});
...
(globalThis as any).MoonCG = MoonCGAPIClient;
