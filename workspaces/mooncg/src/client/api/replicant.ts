...
> extends AbstractReplicant<"client", V, O> {
	 ...
	 */
	_addOperation(
		operation: MoonCG.Replicant.Operation<ReplicantValue<"client", V, O>>,
	): void {
	...
	private _handleOperations(data: {
		...
		namespace: string;
		revision: number;
		operations: MoonCG.Replicant.Operation<ReplicantValue<"client", V, O>>[];
	}): void {
		if (this.status !== "declared") {
			return;
		}
	...
	private _fullUpdate(revision: number): void {
		(window as any).MoonCG.readReplicant(
			this.name,
			this.namespace,
			(data: any) => {
