...
class NcgGraphic extends MutableData(Polymer.PolymerElement) {
	static get template() {
		...
		<style include="mooncg-theme">
			...
			#reloadButton {
				--paper-button: {
					--mooncg-background-color: #6155BD;
				}
			}
			...
			#collapseButton {
				--paper-button: {
					--mooncg-background-color: #525F78;
				}
			}
...
`;
	...
	_computeWorstStatus(instances?: MoonCG.GraphicsInstance[]) {
		if (!instances) {
			return "none";
		}
	...
	_calcCount(singleInstance: boolean, instances?: MoonCG.GraphicsInstance[]) {
		if (singleInstance) {
			return "S";
		}
	...
	_calcReloadAllDisabled(instances?: MoonCG.GraphicsInstance[]) {
