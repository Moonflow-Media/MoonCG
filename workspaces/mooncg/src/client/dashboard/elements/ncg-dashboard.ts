	...
	static get template() {
		...
		<style include="mooncg-theme">
			...
			paper-tabs {
				...
				height: 100%;
				--paper-tabs-selection-bar-color: var(--mooncg-brand-blue);
			...
			paper-tab {
				...
				user-select: none;
				--paper-tab-ink: var(--mooncg-brand-blue);
				--paper-tab-content: {
			...
			paper-icon-button {
				--paper-icon-button-ink-color: var(--mooncg-brand-blue);
				color: white;
			}
			...
			<app-drawer id="drawer" swipe-open="[[smallScreen]]" slot="drawer">
				<template is="dom-if" if="[[smallScreen]]">
					<app-toolbar id="drawerToolbar">
						...
						<img id="drawerLogo" src="/dashboard/img/horiz-logo-2x.png" alt="MoonCG">
					</app-toolbar>
			...
			<app-header-layout fullbleed="">
				...
				<app-header id="header" shadow="" slot="header">
					<app-toolbar id="mainToolbar">
						...
						<img id="mainLogo" src="/dashboard/img/square-logo.png" alt="MoonCG">
		...
		<paper-toast id="reconnectToast" text="Attempting to reconnect to MoonCG server..." duration="0">
...
`;
	...
	override ready(): void {
		...
		window.socket.on("disconnect", () => {
			this.$.mainToast.show("Lost connection to MoonCG server!");
			notified = false;
		...
		window.socket.io.on("reconnect", (attempts) => {
			this.$.mainToast.show("Reconnected to MoonCG server!");
		...
		window.socket.io.on("reconnect_failed", () => {
			this.$.mainToast.show("Failed to reconnect to MoonCG server!");
	...
	_computeDialogs(bundles: MoonCG.Bundle[]) {
		const dialogs: any[] = [];
	...
	_calcButtonClass(buttonType: string) {
		return buttonType === "confirm" ? "mooncg-accept" : "mooncg-reject";
	}
}
