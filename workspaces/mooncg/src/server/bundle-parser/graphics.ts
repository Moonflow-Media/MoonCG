...
export function parseGraphics(
...
): MoonCG.Bundle.Graphic[] {
	const graphics: MoonCG.Bundle.Graphic[] = [];

	if (fs.existsSync(graphicsDir) && typeof manifest.graphics === "undefined") {
		// If the graphics folder exists but the mooncg.graphics property doesn't, throw an error.
		throw new Error(
			`${manifest.name} has a "graphics" folder, ` +
				'but no "mooncg.graphics" property was found in its package.json',
		);
	}

	// If mooncg.graphics exists but the graphics folder doesn't, throw an error.
	if (!fs.existsSync(graphicsDir) && typeof manifest.graphics !== "undefined") {
		throw new Error(
			`${manifest.name} has a "mooncg.graphics" property in its package.json, but no "graphics" folder`,
		);
	}
	...
	manifest.graphics.forEach((graphic, index) => {
		...
		const parsedGraphic: MoonCG.Bundle.Graphic = {
			...graphic,
