...
test("throws if a bundle's `mooncg.mount` property is defined, not an array", () => {
	expect(() => {
		// @ts-expect-error
		parseMounts({ name: "test-bundle", mount: "foo" });
	}).toThrowErrorMatchingInlineSnapshot(
		`[Error: test-bundle has an invalid "mooncg.mount" property in its package.json, it must be an array]`,
	);
});
