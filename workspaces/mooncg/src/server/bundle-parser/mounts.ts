...
export function parseMounts(
	manifest: Pick<MoonCG.Manifest, "mount" | "name">,
): MoonCG.Bundle.Mount[] {
	const mounts: MoonCG.Bundle.Mount[] = [];
	...
	if (!Array.isArray(manifest.mount)) {
		throw new Error(
			`${manifest.name} has an invalid "mooncg.mount" property in its package.json, it must be an array`,
		);
	}
