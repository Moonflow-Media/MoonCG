...
test('when there is a "dashboard" folder but no "dashboardPanels" property, throw an error', () => {
	expect(
		...
		`[Error: no-panels-prop has a "dashboard" folder, but no "mooncg.dashboardPanels" property was found in its package.json]`,
	);
});
...
test("when a panel has a workspace that begins with __mooncg, throw an error", () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/reserved-workspace__mooncg"),
		),
	).toThrowErrorMatchingInlineSnapshot(
		`[Error: Panel "panel.html" in bundle "reserved-workspace__mooncg" is in a workspace whose name begins with __mooncg, which is a reserved string. Please change the name of this workspace to not begin with this string.]`,
	);
});
