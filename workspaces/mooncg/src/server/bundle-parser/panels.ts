...
export function parsePanels(
	...
	// If the dashboard folder exists but the mooncg.dashboardPanels property doesn't, throw an error.
	if (dashboardDirExists && typeof unparsedPanels === "undefined") {
		throw new Error(
			`${bundleName} has a "dashboard" folder, but no "mooncg.dashboardPanels" property was found in its package.json`,
		);
	}
	...
	unparsedPanels?.forEach((panel, index) => {
		...
		if (panel.workspace?.toLowerCase().startsWith("__mooncg")) {
			throw new Error(
				`Panel "${path.basename(panel.file)}" in bundle "${bundleName}" is in a workspace ` +
					"whose name begins with __mooncg, which is a reserved string. Please change the name " +
		...
		let workspaceInfo:
			...
			| {
					dialog: true;
					dialogButtons?: MoonCG.Manifest.UnparsedPanel["dialogButtons"];
			  };
		if (panel.dialog) {
			workspaceInfo = {
				dialog: true,
		...
		const parsedPanel: MoonCG.Bundle.Panel = {
			name: panel.name,
			title: panel.title,
			file: panel.file,
			...sizeInfo,
...
function assertRequiredProps(
	panel: MoonCG.Manifest.UnparsedPanel,
	index: number,
): void {
	const missingProps = [];
