...
test("should error when package.json does not exist", () => {
	expect(parseBundle.bind(parseBundle, testDirPath(""))).toThrowError(
		/^Bundle at path \.\/workspaces(\/|\\)mooncg(\/|\\)test does not contain a package\.json!$/,
	);
});

test('should error when package.json has no "mooncg" property', () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/no-mooncg-prop"),
		),
	).toThrowErrorMatchingInlineSnapshot(
		`[Error: no-mooncg-prop's package.json lacks a "mooncg" property, and therefore cannot be parsed.]`,
	);
});
...
test('should return the expected data when "mooncg" property does exist', () => {
	...
	expect(parsedBundle.homepage).toBe("http://github.com/mooncg");
	...
	expect(parsedBundle.dashboard.panels).toEqual([
		{
			...
			html:
				...
				"\n\t\twindow.parent.dashboardApi = window.mooncg;\n\t</script>\n</body></html>",
			dialog: false,
...
test('should error when "mooncg.compatibleRange" is not a valid semver range', () => {
	expect(
		parseBundle.bind(
			parseBundle,
		...
		`[Error: no-compatible-range's package.json does not have a valid "mooncg.compatibleRange" property.]`,
	);
});
