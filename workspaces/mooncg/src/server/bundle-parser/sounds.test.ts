...
test("should set bundle.soundCues to an empty array when pkg.mooncg.soundCues does not exist", () => {
...
test("should throw an error when pkg.mooncg.soundCues is not an Array", () => {
	expect(() =>
		...
		`[Error: test-bundle's mooncg.soundCues is not an Array]`,
	);
});
...
test("should throw an error when a soundCue lacks a name", () => {
	expect(() =>
		...
		`[Error: mooncg.soundCues[0] in bundle test-bundle lacks a "name" property]`,
	);
});
...
test("should throw an error when a soundCue's default file doesn't exist", () => {
	expect(() =>
		...
		`[Error: mooncg.soundCues[0].defaultFile in bundle test-bundle does not exist]`,
	);
});
