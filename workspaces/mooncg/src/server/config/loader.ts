...
import { mooncgConfigSchema } from "../../types/mooncg-config-schema";
...
export const loadConfig = (cfgDirOrFile: string) => {
	...
	if (userCfg?.bundles?.enabled && userCfg?.bundles?.disabled) {
		throw new Error(
			"mooncg.json may only contain EITHER bundles.enabled OR bundles.disabled, not both.",
		);
	} else if (!userCfg) {
		console.info("[mooncg] No config found, using defaults.");
	}

	const parseResult = mooncgConfigSchema.safeParse(userCfg);

	if (!parseResult.success) {
		console.error(
			"[mooncg] Config invalid:",
	...
	const filteredConfig: MoonCG.FilteredConfig = {
		host: config.host,
		port: config.port,
