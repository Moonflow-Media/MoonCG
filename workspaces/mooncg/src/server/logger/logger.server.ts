...
export function loggerFactory(
	...
	initialOpts.file.path = initialOpts.file.path ?? "logs/mooncg.log";
	...
	const consoleTransport = new winston.transports.Console({
		...
		format: winston.format.combine(
			...
			winston.format.printf(
				(info) =>
					`${initialOpts?.console?.timestamps ? `${String(info.timestamp)} - ` : ""}${info.level}: ${String(info.message)}`,
			),
		),
	});
	...
	const fileTransport = new winston.transports.File({
		...
		format: winston.format.combine(
			...
			winston.format.printf(
				(info) =>
					`${initialOpts?.file?.timestamps ? `${String(info.timestamp)} - ` : ""}${info.level}: ${String(info.message)}`,
			),
		),
	});
