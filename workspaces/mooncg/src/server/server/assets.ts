...
import { sha1File } from "../util/sha1.js";
...
const createAssetFile = (filepath: string, sum: string): MoonCG.AssetFile => {
...
export const assetsRouter = Effect.fn("assetsRouter")(function* (
	...
	const repsByNamespace = new Map<
		...
		Map<
			...
			ServerReplicant<
				MoonCG.AssetFile[],
				MoonCG.Replicant.OptionsWithDefault<MoonCG.AssetFile[]>
			>
		>
	>();
	...
	for (const collection of collections) {
		...
		for (const category of collection.categories) {
			...
			collectionReps.set(
				category.name,
				replicator.declare<MoonCG.AssetFile[]>(
			...
			GroupBy.evaluate((_path, stream) =>
				stream.pipe(
					...
					Stream.runForEach((event) =>
						Effect.tryPromise({
							try: async () => {
								const sum = await sha1File(event.path);
