...
import { sha1FileSync } from "../util/sha1.js";
...
export const soundsRouter = Effect.fn("soundsRouter")(function* (
	...
	const app = express();
	const cueRepsByBundle = new Map<string, ServerReplicant<MoonCG.SoundCue[]>>();
	...
	for (const bundle of bundles) {
		...
		if (bundle.soundCues.length > 0) {
			...
			const cuesRep = replicator.declare<MoonCG.SoundCue[]>(
				"soundCues",
				bundle.name,
				{
	...
	function makeCuesRepDefaultValue(bundle: MoonCG.Bundle): MoonCG.SoundCue[] {
		const formattedCues: MoonCG.SoundCue[] = [];
		for (const rawCue of bundle.soundCues) {
			let file: MoonCG.CueFile | undefined;
			if (rawCue.defaultFile) {
				const filepath = path.join(bundle.dir, rawCue.defaultFile);
				const parsedPath = path.parse(filepath);
				file = {
					sum: sha1FileSync(filepath),
			...
			const formatted: MoonCG.SoundCue = {
				name: rawCue.name,
