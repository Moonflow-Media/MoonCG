...
interface Options {
	standalone?: boolean;
	createApiInstance?: MoonCG.Bundle;
	sound?: boolean;
	fullbleed?: boolean;
}
...
export function injectScripts(
	...
	{ standalone = false, createApiInstance, sound = false }: Options = {},
	...
	function inject(err: NodeJS.ErrnoException | undefined, html: string): void {
		...
		if (resourceType === "panel" || resourceType === "dialog") {
			...
			if (standalone) {
				...
				scripts.push("<script>window.MoonCG = window.top.MoonCG</script>");
			}
		...
		// Inject a small script to create a MoonCG API instance, if requested.
		if (createApiInstance) {
			...
			scripts.push(
				`<script>globalThis.mooncg = new globalThis.MoonCG(${JSON.stringify(
					partialBundle,
