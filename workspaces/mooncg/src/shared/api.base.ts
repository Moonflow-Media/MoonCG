...
export interface MessageHandler {
	messageName: string;
	bundleName: string;
	func: MoonCG.ListenHandler;
}

export abstract class MoonCGAPIBase<
	P extends MoonCG.Platform,
...
> extends TypedEmitter<E> {
	 ...
	 * E.g., `MoonCG.declaredReplicants.myBundle.myRep`
	 */
	 ...
	 * const rep1 = mooncg.Replicant('rep1');
	 * const rep2 = mooncg.Replicant('rep2');
	 *
	 * // You can provide as many Replicant arguments as you want,
	 * // this example just uses two Replicants.
	 * MoonCG.waitForReplicants(rep1, rep2).then(() => {
	...
	/**
	 * The name of the bundle which this MoonCG API instance is for.
	 */
	readonly bundleName: string;

	/**
	 * An object containing the parsed content of `cfg/<bundle-name>.json`, the contents of which
	 * are read once when MoonCG starts up. Used to quickly access per-bundle configuration properties.
	 */
	readonly bundleConfig: DeepReadonly<C>;

	/**
	 * The version (from package.json) of the bundle which this MoonCG API instance is for.
	 * @name MoonCG#bundleVersion
	 */
	 ...
	 */
	readonly bundleGit: Readonly<MoonCG.Bundle.GitData>;
	...
	protected abstract readonly _replicantFactory: <
		V,
		O extends MoonCG.Replicant.Options<any> = MoonCG.Replicant.Options<V>,
	>(
		name: string,
		namespace: string,
		opts: O,
	 ...
	 */
	abstract get Logger(): new (name: string) => MoonCG.Logger;

	/**
	 * An instance of MoonCG's Logger, with the following methods. The logging level is set in `cfg/mooncg.json`,
	 * MoonCG's global config file.
	 * ```
	 * mooncg.log.trace('trace level logging');
	 * mooncg.log.debug('debug level logging');
	 * mooncg.log.info('info level logging');
	 * mooncg.log.warn('warn level logging');
	 * mooncg.log.error('error level logging');
	 * ```
	 */
	abstract get log(): MoonCG.Logger;
	 ...
	 * });
	 */
	listenFor(messageName: string, handlerFunc: MoonCG.ListenHandler): void;
	listenFor(
		messageName: string,
		bundleName: string,
		handlerFunc: MoonCG.ListenHandler,
	): void;
	listenFor(
		messageName: string,
		bundleNameOrHandlerFunc: string | MoonCG.ListenHandler,
		handlerFunc?: MoonCG.ListenHandler,
	): void {
	 ...
	 * @param {function} handlerFunc - A reference to a handler function added as a listener to this message via {@link MoonCG#listenFor}.
	 * @returns {boolean}
	 *
	 * @example
	 * mooncg.unlisten('printMessage', someFunctionName);
	 *
	 * @example <caption>Removing a listener from a message in another bundle's namespace:</caption>
	 * mooncg.unlisten('printMessage', 'another-bundle', someFunctionName);
	 */
	unlisten(messageName: string, handlerFunc: MoonCG.ListenHandler): boolean;
	unlisten(
		messageName: string,
		bundleName: string,
		handlerFunc: MoonCG.ListenHandler,
	...
	unlisten(
		messageName: string,
		bundleNameOrHandler: string | MoonCG.ListenHandler,
		maybeHandler?: MoonCG.ListenHandler,
	): boolean {
		let { bundleName } = this;
	 ...
	 *
	 * By default Replicants will be saved to disk, meaning they will automatically be restored when MoonCG is restarted,
	 ...
	 *
	 * As of MoonCG 0.8.4, Replicants can also be automatically validated against a JSON Schema that you provide.
	 ...
	 * Defaults to `mooncg/bundles/${bundleName}/schemas/${replicantName}.json`. Please note that this default
	...
	Replicant<
		V,
		O extends MoonCG.Replicant.OptionsNoDefault =
			MoonCG.Replicant.OptionsNoDefault,
	...
	Replicant<
		V,
		O extends MoonCG.Replicant.OptionsWithDefault<V> =
			MoonCG.Replicant.OptionsWithDefault<V>,
	>(name: string, namespace: string, opts?: O): AbstractReplicant<P, V, O>;
	Replicant<
		V,
		O extends MoonCG.Replicant.OptionsWithDefault<V> =
