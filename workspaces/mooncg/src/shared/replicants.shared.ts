...
interface Events<P extends MoonCG.Platform, V, O, S extends boolean> {
	change: (
		newVal: ReplicantValue<P, V, O, S>,
		oldVal: ReplicantValue<P, V, O, S> | undefined,
		operations: MoonCG.Replicant.Operation<ReplicantValue<P, V, O, S>>[],
	) => void;
	declared: (
		data:
...
> extends TypedEmitter<Events<P, V, O, S>> {
	...
	protected _operationQueue: MoonCG.Replicant.Operation<
	...
	constructor(
		...
		// https://github.com/mooncg/mooncg/issues/296
	 ...
	 */
	_applyOperation(operation: MoonCG.Replicant.Operation<V>): boolean {
		ignoreProxy(this);
		...
		if (ARRAY_MUTATOR_METHODS.includes(operation.method)) {
			...
			const arr: unknown = objectPath.get(this.value, path);
	 ...
	 * @private
	 */
	abstract _addOperation(operation: MoonCG.Replicant.Operation<V>): void;
...
export function ignoreProxy(
	replicant: AbstractReplicant<any, any, MoonCG.Replicant.Options<any>, any>,
): void {
