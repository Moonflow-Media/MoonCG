...
const parsedArgv = argvSchema.parse(
	parseArgs({
		options: {
			bundlesEnabled: { type: "string" },
			bundlesDisabled: { type: "string" },
			bundlesPaths: { type: "string" },
		},
		strict: false,
	}).values,
);

export const mooncgConfigSchema = z
	.object({
		host: z
			.string()
			.default("0.0.0.0")
			.describe("The IP address or hostname that MoonCG should bind to."),
		port: z
			.number()
			.int()
			.positive()
			.default(9090)
			.describe("The port that MoonCG should listen on."),
		baseURL: z
			.string()
			.optional()
			.describe(
...
export type MoonCGConfig = z.infer<typeof mooncgConfigSchema>;
