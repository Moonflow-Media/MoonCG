...
import type { MoonCGConfig } from "./mooncg-config-schema";
import type {
...
export namespace MoonCG {
	/**
	 * A collection of types that represent the raw data from the `mooncg` stanza in a bundle's `package.json`.
	 */
	export namespace Manifest {
		...
		/**
		 * The actual structure of a bundle's `mooncg` stanza from it's `package.json`.
		 */
	 ...
	 * Only includes types that MoonCG cares about.
	 */
	export interface PackageJSON {
		...
		author?: Person;
		contributors?: Person[];
		mooncg?: Manifest.UnparsedManifest;
	}

	/**
	 * A _parsed_ bundle manifest as used internally by MoonCG.
	 */
	export type Manifest = Omit<PackageJSON, "mooncg"> &
	...
	export interface MooncgBundleConfig {
	...
	export interface Bundle {
		...
		mooncgBundleConfig: MooncgBundleConfig;
	}

	/**
	 * MoonCG's socket protocol.
	...
	/**
	 * The full, parsed MoonCG config (i.e `cfg/mooncg.{json,yaml,js}`).
	 * Contains sensitive secrets!
	 * Available to server-side code (extensions) only.
	 */
	export type Config = MoonCGConfig;

	/**
	 * The filtered, minimal, parsed MoonCG config (i.e `cfg/mooncg.{json,yaml,js}`).
	...
	/**
	 * Express middleware that can be mounted to MoonCG's webserver via `mooncg.mount`.
	 */
	...
	export type Acknowledgement =
		HandledAcknowledgement | UnhandledAcknowledgement;
	...
	/**
	 * A description of MoonCG's logger interface.
	 */
	...
	/**
	 * The logging levels available to MoonCG.
	 */
	...
	/**
	 * A description of which platform/environment an instance of the MoonCG API is being used in.
