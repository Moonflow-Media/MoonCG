...
import { getConnection } from "@mooncg/database-adapter-sqlite-legacy";
...
export async function setupTest(mooncgConfigName = "mooncg.json") {
	const tmpDir = createTmpDir();

	// Tell MoonCG to look in our new temp folder for bundles, cfg, db, and assets, rather than whatever ones the user
	// may have. We don't want to touch any existing user data!
	process.env.MOONCG_ROOT = tmpDir;

	fs.cpSync(
		testDirPath("fixtures/mooncg-core/assets"),
	...
	fs.cpSync(
		testDirPath("fixtures/mooncg-core/bundles"),
	...
	fs.cpSync(testDirPath("fixtures/mooncg-core/cfg"), path.join(tmpDir, "cfg"), {
		recursive: true,
	});
	fs.cpSync(
		testDirPath(`fixtures/mooncg-core/cfg/${mooncgConfigName}`),
