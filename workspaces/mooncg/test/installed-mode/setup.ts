...
import type { getConnection } from "@mooncg/database-adapter-sqlite-legacy";
...
interface TestServerWrapper {
	...
	handle: ServerHandle;
	bundleService: ServerHandle["bundleService"];
}
...
export async function setupInstalledModeTest(mooncgConfigName = "mooncg.json") {
	const tmpDir = createTmpDir();

	// Copy test-bundle contents directly to tmpDir root (tmpDir IS the bundle)
	const testBundlePath = testDirPath(
		"fixtures/mooncg-core/bundles/test-bundle",
		true,
	);
	...
	// Copy MoonCG configuration
	fs.cpSync(
		testDirPath("fixtures/mooncg-core/cfg", true),
	...
	fs.cpSync(
		testDirPath(`fixtures/mooncg-core/cfg/${mooncgConfigName}`, true),
		path.join(tmpDir, "cfg/mooncg.json"),
		{ recursive: true },
	);

	// Copy test assets (bundle's assets)
	fs.cpSync(
		testDirPath("fixtures/mooncg-core/assets", true),
	...
	// Copy entire node_modules directory (includes all dependencies and @mooncg/* workspaces)
	fs.cpSync(
	...
	// Create node_modules/mooncg and copy the current MoonCG project into it
	const mooncgModulePath = path.join(tmpDir, "node_modules/mooncg");

	// Change cwd and set MOONCG_ROOT before importing MoonCGServer
	...
	const serverModulePath = pathToFileURL(
		path.join(mooncgModulePath, "src/server/server/index.ts"),
	).href;
	...
	const server: TestServerWrapper = {
		...
		get bundleService() {
			return serverHandle.bundleService;
		},
	};
	...
	return test
		...
		.extend<Pick<SetupContext, "database">>({
			database: async ({}, use) => {
				const { getConnection } =
					await import("@mooncg/database-adapter-sqlite-legacy");
