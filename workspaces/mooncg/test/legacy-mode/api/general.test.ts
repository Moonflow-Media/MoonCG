...
import { MoonCG } from "../../../src/types/mooncg";
...
test("should mount custom routes via mooncg.mount", async ({ apis }) => {
	const app = express();
...
test("should mount prefixed custom routes via mooncg.mount", async ({
	apis,
}) => {
	const app = express();
...
test("should mount custom routes via the built-in router and mooncg.mount", async ({
	apis,
}) => {
...
test("should prevent acknowledgements from being called more than once", async ({
	...
	const promise = Promise.all([
		new Promise<MoonCG.Acknowledgement | undefined>((resolve) => {
...
test("bundles replicant", ({ apis }) => {
	const bundlesRep = apis.extension.Replicant<MoonCG.Bundle[]>(
		"bundles",
		"mooncg",
	);
