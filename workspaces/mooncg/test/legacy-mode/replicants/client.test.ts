import { Replicant } from "@mooncg/database-adapter-sqlite-legacy";
	...
	const ret = await dashboard.evaluate(
		async () =>
			new Promise<{
				newVal: unknown[];
				oldVal: unknown[];
				operations: MoonCG.Replicant.Operation<unknown[]>[];
			}>((resolve) => {
	...
	const ret = await dashboard.evaluate(
		async () =>
			new Promise<{
				newVal: RepType;
				oldVal: RepType;
				operations: MoonCG.Replicant.Operation<RepType>[];
			}>((resolve) => {
...
test("persistent - should persist changes to disk", async ({
	...
	await dashboard.evaluate(async () => {
		...
		await window.MoonCG.waitForReplicants(rep);
	...
	await dashboard.evaluate(
		async () =>
			new Promise((resolve) => {
				...
				(window as any).MoonCG.waitForReplicants(rep1, rep2, rep3).then(
					resolve,
				);
			}),
	);
